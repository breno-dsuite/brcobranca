...
RSpec.describe Brcobranca::Retorno::Cnab400::BancoBrasil do
  before do
    @arquivo = File.join(File.dirname(__FILE__), '..', '..', '..', 'arquivos', 'CNAB400BANCOBRASIL.RET')
  end
  ...
  it 'Transforma arquivo de retorno em objetos de retorno retornando somente as linhas de pagamentos de títulos sem registro' do
    ...
    pagamento = pagamentos.first
    expect(pagamento.agencia_com_dv).to eql('15857')
    expect(pagamento.cedente_com_dv).to eql('000298700')
    expect(pagamento.nosso_numero).to eql('32131030000000019')
    expect(pagamento.carteira_variacao).to eql('019')
    expect(pagamento.carteira).to eql('17')
    expect(pagamento.data_vencimento).to eql('130121')
    expect(pagamento.valor_titulo).to eql('0000000181685')
    expect(pagamento.banco_recebedor).to eql('001')
    expect(pagamento.agencia_recebedora_com_dv).to eql('03727')
    expect(pagamento.especie_documento).to eql('01')
    expect(pagamento.valor_tarifa).to eql('0000000')
    ...
    expect(pagamento.valor_recebido).to eql('0000000000000')
    ...
    expect(pagamento.data_ocorrencia).to eql('111220')
    expect(pagamento.data_credito).to eql('000000')
